...
switch ($action) {
  case 'visaoGeral':
    ...
    $condicao = "NNOA.CD_CENTRO_CUSTO = $idCentroCusto AND NNOA.DT_ANO = '$ano' AND SCD.CD_ORIGEM = 20 AND SCOC.CD_USUARIO_AUTORIZOU IS NOT NULL AND SCOC.CD_STATUS = 2 AND MONTH(DT_AUTORIZACAO) = '$mes'";
    if ($filial > 0) {
  ...
  case 'detalhes':
    ...
    $mes = $_POST['mes'];
    $filial = $_SESSION['filial'];
    $condicao = "SCOC.CD_USUARIO_AUTORIZOU IS NOT NULL AND YEAR(SCOC.DT_ATUALIZACAO) = '$ano' AND MONTH(SCOC.DT_ATUALIZACAO) = '$mes' AND SCD.CD_CENTRO_CUSTO = $idCentroCusto AND SCD.DS_CONTA_GERENCIAL IS NOT NULL";
    if ($filial > 0) {
